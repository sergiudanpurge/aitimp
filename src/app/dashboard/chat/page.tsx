...
export default function ChatPage() {
  ...
  const ConvList = () => (
    <div style={{ borderRight: isMobile ? "none" : "1px solid #262626", display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ padding: "16px", borderBottom: "1px solid #262626" }}>
        <input placeholder="Caută conversație..." style={{ width: "100%", padding: "9px 14px", background: "#1e1e1e", border: "1px solid #262626", borderRadius: 8, color: "#f0ede8", fontSize: 13, outline: "none", fontFamily: "var(--font-outfit)", boxSizing: "border-box" as const }} />
      </div>
      <div style={{ flex: 1, overflowY: "auto" }}>
        {mockConversations.map(c => (
          <div key={c.id} onClick={() => selectConv(c.id)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", cursor: "pointer", background: selected === c.id ? "rgba(201,169,110,0.08)" : "transparent", borderLeft: `3px solid ${selected === c.id ? "#c9a96e" : "transparent"}`, transition: "all 0.15s" }}>
            <div style={{ width: 44, height: 44, borderRadius: "50%", background: "linear-gradient(135deg,#c9a96e,#8b5e3c)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, fontWeight: 700, color: "#fff", flexShrink: 0, fontFamily: "var(--font-playfair)" }}>
              {c.avatar}
            </div>
            ...
            {c.unread > 0 && (
              <div style={{ width: 20, height: 20, borderRadius: "50%", background: "#c9a96e", color: "#0a0a0a", fontSize: 10, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                {c.unread}
              </div>
            )}
          </div>
  ...
  const MessagesPanel = () => (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ padding: "14px 20px", borderBottom: "1px solid #262626", display: "flex", alignItems: "center", gap: 12 }}>
        {isMobile && (
          <button onClick={() => setShowList(true)} style={{ background: "none", border: "none", color: "#777", fontSize: 20, cursor: "pointer", padding: "0 4px" }}>←</button>
        )}
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px", display: "flex", flexDirection: "column", gap: 10 }}>
        {(messages[selected!] || []).map((m: any, i: number) => (
          <div key={i} style={{ display: "flex", justifyContent: m.from === "me" ? "flex-end" : "flex-start" }}>
            <div style={{ maxWidth: isMobile ? "80%" : "70%", padding: "10px 14px", borderRadius: m.from === "me" ? "14px 14px 4px 14px" : "14px 14px 14px 4px", background: m.from === "me" ? "linear-gradient(135deg,#c9a96e,#a8843d)" : "#1e1e1e", color: m.from === "me" ? "#0a0a0a" : "#f0ede8", fontSize: 13, lineHeight: 1.5 }}>
              <div>{m.text}</div>
      ...
      <div style={{ padding: "12px 16px", borderTop: "1px solid #262626", display: "flex", gap: 8 }}>
  ...
  return (
    <DashboardLayout title="Chat">
      <div style={{ background: "#161616", border: "1px solid #262626", borderRadius: 14, overflow: "hidden", height: isMobile ? "calc(100vh - 130px)" : "calc(100vh - 160px)", display: "grid", gridTemplateColumns: isMobile ? "1fr" : "280px 1fr" }}>
        {isMobile ? (
          showList ? <ConvList /> : <MessagesPanel />
        ) : (
          <>
            <ConvList />
            <MessagesPanel />

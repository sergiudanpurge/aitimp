  ...
  return (
    <DashboardLayout title="Recenzii">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: 20 }}>
        <div style={{ background: "#161616", border: "1px solid #262626", borderRadius: 14, padding: "28px 36px", textAlign: "center", minWidth: 160 }}>
          <div style={{ fontFamily: "var(--font-playfair)", fontSize: 52, fontWeight: 700, color: "#c9a96e", lineHeight: 1 }}>{avg.toFixed(1)}</div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {mockReviews.filter(r => filter === "toate" || r.rating === parseInt(filter)).map((r, i) => (
          <div key={i} style={{ background: "#161616", border: "1px solid #262626", borderRadius: 14, padding: "20px 24px", transition: "border-color 0.2s" }}
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                  <div>
                    ...
                    <span style={{ fontSize: 11, color: "#777", marginLeft: 10 }}>· {r.service}</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ color: "#e8b84b", fontSize: 13 }}>{"★".repeat(r.rating)}{"☆".repeat(5 - r.rating)}</span>
                    <span style={{ fontSize: 11, color: "#777" }}>{r.date}</span>
                  </div>
                </div>
                <p style={{ fontSize: 13, color: "#a0a0a0", lineHeight: 1.6, fontStyle: "italic" }}>"{r.text}"</p>
              </div>
            </div>
          </div>
        ))}
